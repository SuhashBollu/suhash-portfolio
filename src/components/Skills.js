...
const programmingSkillCards = programmingSkills.map((skill) => (
  <SkillCard name={skill.name} image={skill.image} level={skill.level} />
));
const webSkillCards = webSkills.map((skill) => (
  <SkillCard name={skill.name} image={skill.image} level={skill.level} />
));
const dbSkillCards = dbSkills.map((skill) => (
  <SkillCard name={skill.name} image={skill.image} level={skill.level} />
));
const fwSkillCards = frameworks.map((skill) => (
  <SkillCard name={skill.name} image={skill.image} level={skill.level} />
));
const otherSkillCards = otherSkills.map((skill) => (
  <SkillCard name={skill.name} image={skill.image} level={skill.level} />
));
...
const skills = (props) => (
  <div className="Skills">
    ...
    <h3>Programming</h3>
    <div className="row">{programmingSkillCards}</div>
    {/* </div> */}
    <h3>Web</h3>
    <div className="row">{webSkillCards}</div>
    <h3>Database</h3>
    <div className="row">{dbSkillCards}</div>
    <h3>Frameworks</h3>
    <div className="row">{fwSkillCards}</div>
    <h3>Other Tools</h3>
    <div className="row">{otherSkillCards}</div>
  </div>
);
